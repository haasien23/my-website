...
const OUT_DIR = path.join(ROOT, "output", "defense_ppt_previous_blue_synced");
const SLIDE_DIR = path.join(OUT_DIR, "slides");
const DESKTOP = path.join(os.homedir(), "Desktop");
const PPTX_PATH = path.join(DESKTOP, "基于集群的多任务协同态势感知平台_本科毕设答辩_江理红棕校园模板版.pptx");
const TARGET_PPTX_PATH = path.join(DESKTOP, "基于集群的多任务协同态势感知平台_本科毕设答辩.pptx");
const FALLBACK_PPTX_PATH = path.join(DESKTOP, "基于集群的多任务协同态势感知平台_本科毕设答辩_RADS大字修订版.pptx");
const COPY_PATH = path.join(OUT_DIR, path.basename(TARGET_PPTX_PATH));
...
const slides = [
  ...
  },
  {
    ...
    html: slideShell(13, "RADS 原理三：动态预算控制每一步的总派机规模", "预算机制是 RADS 降低资源投入的关键，它决定本步最多投入多少架无人机。", `
      <div class="budget-principle">
        <section class="budget-left" data-fit>
          ${formula("D(t)=0.25M+0.75R<sub>Σ</sub>(t)+0.85H(t)+0.60E<sub>avg</sub>(t)+0.40L<sub>avg</sub>(t)+F(t)", "D(t) 是“期望派机量”：任务越重、热点越多，D(t) 越大；系统状态越健康，可支撑的预算越高。")}
          ${formula("B(t)=min(B<sub>cap</sub>, max(B<sub>min</sub>, round(D(t))))", "把 D(t) 四舍五入成整数，再限制在最小预算和可用节点上限之间。")}
          <div class="budget-factors explained">
            <div><b>M / R<sub>Σ</sub></b><span>目标数量与总紧急度，表示当前任务压力。</span></div>
            <div><b>H(t)</b><span>热点目标数：需求≥3 或紧急度≥0.82 的目标。</span></div>
            <div><b>E<sub>avg</sub> / L<sub>avg</sub></b><span>可用无人机平均能量与平均链路质量。</span></div>
            <div><b>F(t)</b><span>近期成功率低时补预算，成功率高时略收缩。</span></div>
          </div>
          <div class="budget-eq">一句话：先算“应该派多少”，再用上下限把它变成可执行预算。</div>
        </section>
        <section class="budget-center" data-fit>
          ...
          <svg viewBox="0 0 520 320" class="budget-svg">
            ...
            <line x1="260" y1="250" x2="354" y2="136" stroke="#082b5f" stroke-width="8" stroke-linecap="round"/>
            <circle cx="260" cy="250" r="12" fill="#082b5f"/>
            <rect x="150" y="118" width="240" height="112" rx="12" fill="#fffdf9" opacity="0.96"/>
            <text x="260" y="178" text-anchor="middle" class="gauge-num">B(t)=9</text>
            <text x="260" y="213" text-anchor="middle" class="gauge-label">本步派机预算</text>
            <text x="76" y="286" class="gauge-tick">保守</text>
            <text x="420" y="286" class="gauge-tick">积极</text>
          </svg>
          <div class="budget-caption">预算不是固定比例，而是每个仿真步根据任务压力、节点状态和近期表现滚动更新。</div>
        </section>
    ...
    notes: [
      ...
      "D(t) 可以理解成期望派机量，里面包含目标数量、总紧急度、热点目标数量、平均能量、平均链路质量和近期成功反馈。",
      "B(t) 则是最终可执行预算：先把 D(t) 四舍五入，再限制在最小预算和可用节点上限之间。",
      "这就是 RADS 能够降低能耗和派机规模的关键。它不是盲目少派，而是根据场景压力动态决定本步派机上限。",
    ],
  },
  {
    title: "RADS 原理四",
    html: slideShell(14, "RADS 原理四：预算约束下的贪心式子群生成", "在每一步预算范围内，优先服务高紧急目标，并选择当前效用最高的可用无人机。", `
      <div class="greedy-principle">
        <section class="pseudo dense" data-fit>
          <h3>贪心式子群生成：从“目标需求”到“无人机名单”</h3>
          <ol>
            <li>构造可用集合 A(t)：未故障、能量高于阈值的无人机。</li>
            <li>对每个目标计算需求 N<sub>j</sub><sup>dem</sup>(t) 与紧急度 R<sub>j</sub>(t)，按 R<sub>j</sub> 降序排队。</li>
            <li>计算全局预算 B(t)，它是本步所有目标共享的派机上限。</li>
            <li>轮到某个目标时，只从“还没被派走”的无人机里挑。</li>
            <li>谁最适合，就加入这个目标的小队；预算减 1。目标够人、预算用完或没有合适无人机时停止。</li>
          </ol>
          <p class="code-map">代码映射：assign_smart()</p>
          <div class="greedy-formula">
            <b>当前目标子群 = 原来的子群 + 本轮选中的无人机</b>
            <span>本轮选中的无人机：还没被派走，并且综合评分最高。</span>
          </div>
        </section>
        <section class="assign-visual" data-fit>
          ...
          <div class="assign-grid">
            ...
            <span>T2 高紧急</span><span>3架</span><span>U09 0.91 · U14 0.74 · U03 0.62</span><strong>U09、U14、U03</strong>
            <span>T0 中紧急</span><span>2架</span><span>U03 已占用 · U06 0.68 · U11 0.61</span><strong>U06、U11</strong>
            <span>T5 低紧急</span><span>2架</span><span>预算已用完，进入下一步滚动重算</span><strong class="muted">暂缓</strong>
          </div>
          <div class="budget-state">
            <div><b>预算 B(t)</b><span>5 架</span></div>
            <div><b>已选节点</b><span>5 架</span></div>
            <div><b>剩余预算</b><span>0 架</span></div>
          </div>
          <div class="assign-explain">
            <div><b>先服务热点</b><span>T2 紧急度最高，优先获得高效用无人机。</span></div>
            <div><b>避免重复占用</b><span>U03 已被选择后，不再重复分配给 T0。</span></div>
            <div><b>滚动补偿</b><span>T5 本步暂缓，下一仿真步会重新计算需求、效用和预算。</span></div>
          </div>
        </section>
        <section class="subgroup-output" data-fit>
          ...
          <div class="greedy-summary">
            <b>核心理解</b>
            <span>子群生成不是固定分组，而是“每步重算、目标排队、按效用补齐、受预算截断”的在线决策。</span>
          </div>
        </section>
      </div>
    `),
    notes: [
      ...
      "这一页可以通俗讲成排队补人：先让更紧急的目标排在前面，轮到某个目标时，只从还没被派走的无人机里挑。",
      "挑选标准就是综合评分，谁更适合当前目标，就把谁加入这个目标的子群，同时预算减一。",
  ...
  },
  {
    ...
    html: slideShell(15, "感知融合与严格成功：覆盖只是开始，融合结果才是最终评价", "平台中的成功判定同时要求观测确认、定位达标和一致性达标，因此比普通覆盖率更严格。", `
      <div class="fusion-layout">
        <section>
          ...
          ${formula("w<sub>i</sub>=((0.55+0.25L<sub>i</sub>+0.20E<sub>i</sub>)/σ<sub>i</sub><sup>2</sup>)·(0.78+0.22L<sub>i</sub>)·0.90<sup>age</sup>·ρ<sub>i</sub>", "权重由测量噪声、链路质量、剩余能量、信息年龄和离群惩罚共同决定。")}
          ${formula("P<sub>est,j</sub>(t)=Σw<sub>i</sub>P<sub>ij</sub>(t)/Σw<sub>i</sub>, i∈S<sub>j</sub>", "融合位置是子群 S_j 内多架无人机观测的加权平均，高质量观测占更大权重。")}
          ${formula("Success<sub>j</sub>=Confirmed<sub>j</sub> ∧ ||P<sub>est,j</sub>-P<sub>j</sub>||≤32m ∧ Consensus<sub>j</sub>≥0.42", "严格成功不是“看见了”就算成功，而是确认数量、定位误差和一致性同时达标。")}
          ${panel("融合逻辑", bullets([
    ...
    notes: [
      ...
      "融合位置采用加权平均，权重由测量噪声、链路质量、剩余能量、信息年龄和离群程度共同决定。质量更高、链路更可靠、延迟更小的观测占更大权重。",
      "最后严格成功需要确认数量、定位误差和一致性同时满足，所以这个指标比普通覆盖率更严格，也更能反映真实协同感知质量。",
    ],
  },
  {
    title: "RADS 原理总结",
    html: slideShell(16, "RADS 原理总结：按需派机与滚动感知闭环", "方法核心由目标需求、节点评分、预算约束与融合评价共同构成。", `
      <div class="rads-summary-layout">
        <section class="rads-summary-main" data-fit>
          <div class="rads-loop-intro">
            <b>核心概括：按需派机闭环</b>
            <span>目标需要什么、无人机适合什么、当前预算允许多少，每一步都重新判断。</span>
          </div>
          <div class="rads-loop-map">
            <div class="loop-node n1"><b>01</b><strong>目标需求</strong><span>谁更需要被观测</span></div>
            <div class="loop-node n2"><b>02</b><strong>节点评分</strong><span>谁更适合去观测</span></div>
            <div class="loop-core"><b>RADS</b><span>滚动调度</span></div>
            <div class="loop-node n3"><b>03</b><strong>预算约束</strong><span>本步最多派几架</span></div>
            <div class="loop-node n4"><b>04</b><strong>子群生成</strong><span>选出最合适小队</span></div>
            <div class="loop-node n5"><b>05</b><strong>融合评价</strong><span>回传后估计目标状态</span></div>
          </div>
          <div class="rads-loop-note">
            <b>方法要点：</b>
            <span>RADS 不是只看距离，也不是固定派机；它每一步都根据目标、节点和预算重新形成感知子群。</span>
          </div>
        </section>
        <section class="rads-summary-side" data-fit>
          <div class="rads-name">
            <h3>RADS = Risk-Aware Dynamic Scheduling</h3>
            <p>风险感知动态调度</p>
          </div>
          <div class="rds-grid">
            <div><b>R</b><strong>Risk</strong><span>判断哪些目标更值得投入资源。</span></div>
            <div><b>D</b><strong>Dynamic</strong><span>每个仿真步根据状态重新计算。</span></div>
            <div><b>S</b><strong>Scheduling</strong><span>在预算内选择合适无人机。</span></div>
          </div>
          <div class="defense-answer">
            <h3>方法原理概括</h3>
            <p>RADS 是一种面向无人机集群协同感知的按需派机方法。每个仿真步先评估目标需求，再计算无人机与目标之间的适配分数；随后根据任务压力和节点状态确定本步预算，并在预算内选择评分较高且未被占用的无人机组成子群。该方法在保持感知质量的同时控制资源投入。</p>
          </div>
          <div class="code-map">代码映射：compute_target_meta / compute_utility / compute_adaptive_budget / assign_smart</div>
        </section>
      </div>
    `),
    notes: [
      "这一页用于在 RADS 原理讲完后做总收束。不要按论文式长句背，先说它是一种按需派机方法。",
      "核心话术是：每个仿真步先评估目标需求，再计算无人机和目标之间的适配分数；系统根据任务压力和节点状态确定预算，最后在预算内选择评分较高且未被占用的无人机组成子群。",
      "RADS 的 R 是风险，表示哪些目标更值得投入资源；D 是动态，表示每一步都会重新计算；S 是调度，表示决定派哪几架无人机。",
      "最后补一句：与全量派遣相比，它控制资源投入；与随机派遣相比，它利用目标需求和节点状态信息，观测回来以后还要加权融合。",
    ],
  },
  {
    title: "平台实现",
  ...
  <head>
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      .budget-left .formula { margin: 0 0 14px; padding: 15px 17px; }
      .budget-left .formula > div { font-size: 25px; line-height: 1.22; }
      .budget-left .formula p { font-size: 20px; line-height: 1.28; }
      ...
      .budget-factors.explained { grid-template-columns: 1fr 1fr; gap: 11px; }
      .budget-factors.explained div { min-height: 92px; padding: 12px 14px; }
      .budget-factors.explained b { font-size: 23px; margin-bottom: 7px; }
      .budget-factors.explained span { font-size: 20px; line-height: 1.26; }
      .budget-eq { margin-top: 14px; padding: 15px; background: ${C.navy}; color: white; font-size: 23px; line-height: 1.25; font-weight: 800; }
      ...
      .greedy-principle {
        display: grid;
        grid-template-columns: 0.96fr 1.1fr 0.88fr;
        gap: 24px;
        height: 100%;
      }
      ...
      .pseudo.dense h3 { font-size: 26px; margin-bottom: 12px; }
      .pseudo.dense ol { padding-left: 30px; }
      .pseudo.dense li { font-size: 20px; line-height: 1.34; margin: 8px 0; }
      .greedy-formula {
        margin-top: 16px;
        padding: 17px 18px;
        background: ${C.pale};
      ...
      .greedy-summary b { display: block; color: ${C.navy}; font-size: 22px; margin-bottom: 7px; }
      .greedy-summary span { display: block; color: ${C.ink}; font-size: 19px; line-height: 1.28; }
      .assign-explain {
        margin-top: 18px;
        display: grid;
        grid-template-columns: repeat(3, 1fr);
      ...
      .fusion-layout .formula { padding: 10px 13px; margin-bottom: 9px; border-left-width: 8px; }
      .fusion-layout .formula > div { font-size: 21px; line-height: 1.22; }
      .fusion-layout .formula p { font-size: 17px; line-height: 1.25; }
      ...
      .rads-summary-main, .rads-summary-side {
        border: 2px solid ${C.line};
        border-radius: 8px;
        background: white;
        padding: 24px 28px;
      ...
      .rads-loop-map {
        ...
        border: 1px solid ${C.line};
        background:
          radial-gradient(circle at center, rgba(143,29,29,.045), transparent 38%),
          ${C.pale};
        overflow: hidden;
      }
      .rads-loop-map::before {
        content: "";
        position: absolute;
        left: 120px;
        right: 120px;
        top: 66px;
        bottom: 54px;
        border: 4px dashed rgba(143,29,29,.28);
        border-radius: 50%;
      }
      .loop-core {
        position: absolute;
        ...
        justify-content: center;
        align-items: center;
        box-shadow: 0 12px 26px rgba(91,23,23,.18);
        z-index: 3;
      }
      .loop-core b {
        font-size: 48px;
      ...
      .loop-node {
        ...
        box-shadow: 0 8px 18px rgba(91,23,23,.06);
        z-index: 2;
      }
      .loop-node b {
        display: inline-flex;
      ...
      .loop-node.n1 { left: 50%; top: 20px; transform: translateX(-50%); }
      .loop-node.n2 { left: 26px; top: 158px; }
      .loop-node.n3 { right: 26px; top: 158px; }
      .loop-node.n4 { right: 128px; bottom: 24px; }
      .loop-node.n5 { left: 128px; bottom: 24px; }
      .loop-node.n4 b, .loop-node.n2 b { background: ${C.red}; }
      .rads-loop-note {
        padding: 18px 20px;
        border-left: 9px solid ${C.red};
        background: ${C.pale};
      }
      .rads-loop-note b {
        display: block;
      ...
      .rads-pipeline div:nth-child(even) { background: #fffdf9; }
      .rads-pipeline b {
        display: flex;
        align-items: center;
        justify-content: center;
      ...
      /* Jiangxi University blue academic skin for the previously generated deck. Content stays unchanged; this layer only replaces the visual system. */
      .slide {
        background:
          linear-gradient(120deg, rgba(8,43,95,.035) 0 1px, transparent 1px 28px),
          radial-gradient(circle at 88% 8%, rgba(201,154,58,.13), transparent 18%),
          linear-gradient(135deg, rgba(255,255,255,.98), rgba(247,251,255,.98)),
          #f8fbff;
      }
      .slide::before {
        height: 18px;
        background: linear-gradient(90deg, ${C.navy} 0 76%, ${C.red} 76% 86%, ${C.gold} 86% 100%);
      }
      .slide::after {
        content: "JXUST";
        right: 92px;
        bottom: 62px;
        color: rgba(8,43,95,.05);
        font-size: 118px;
        letter-spacing: 10px;
      }
      .jxust-bg {
      ...
      .jxust-bg::before {
        ...
        width: 430px;
        height: 210px;
        background: linear-gradient(135deg, transparent 0 29%, rgba(201,154,58,.32) 29% 30.4%, rgba(8,43,95,.10) 30.4% 100%);
        opacity: .82;
      }
      .jxust-bg::after {
        content: "";
      ...
      .jxust-bg-motto {
        right: 24px;
        top: 280px;
        color: rgba(8,43,95,.12);
      }
      .jxust-bg-campus {
        bottom: 52px;
        opacity: .36;
        border-bottom-color: rgba(8,43,95,.18);
      }
      .topbar {
      ...
      .footer {
        color: #6e7a88;
        border-top: 1px solid rgba(8,43,95,.16);
      }
      .panel, .mini-panel, .chart-card, .image-box, .metric, .problem-core, .compare-box, .question-box,
      .req-item, .agenda-row, .agenda-side, .thesis-map, .rads-key, .rads-theory-strip, .demand-flow,
      .subgroup-diagram, .result-note, .thanks, .thanks-full,
      ...
      .comm-panel {
        background: rgba(255,255,255,.965);
        border-color: ${C.line};
        box-shadow: 0 12px 26px rgba(8,43,95,.055);
      }
      ...
      .grid-line { stroke: ${C.line2}; }
      .axis-line { stroke: ${C.line}; }
      .cover {
        padding: 0;
        background:
          linear-gradient(120deg, rgba(8,43,95,.035) 0 1px, transparent 1px 30px),
      ...
      .cover-diagonal.red {
        ...
        width: 420px;
        height: 390px;
        background: linear-gradient(135deg, rgba(8,43,95,.94), rgba(18,63,120,.82));
      ...
      .cover-title-simple h1 {
        ...
        letter-spacing: 0;
        padding: 18px 0 18px;
        border-top: 2px solid rgba(8,43,95,.24);
        border-bottom: 2px solid rgba(8,43,95,.24);
      }
      .cover-subtitle {
      ...
      .cover-meta-simple div {
        min-height: 54px;
        border-left: none;
        border-top: 2px solid rgba(8,43,95,.22);
        padding: 11px 0 0 0;
      }
      .cover-meta-simple b {
        color: ${C.red};
...
async function buildPptx() {
  ...
  let writtenPath = TARGET_PPTX_PATH;
  try {
    await pptx.writeFile({ fileName: TARGET_PPTX_PATH, compression: true });
  } catch (err) {
    if (!["EBUSY", "EPERM", "EACCES"].includes(err && err.code)) throw err;
    writtenPath = FALLBACK_PPTX_PATH;
    await pptx.writeFile({ fileName: writtenPath, compression: true });
  }
  globalThis.__LAST_WRITTEN_PPTX_PATH = writtenPath;
  fs.copyFileSync(writtenPath, COPY_PATH);
}

async function inspectPptx() {
  const writtenPath = globalThis.__LAST_WRITTEN_PPTX_PATH || TARGET_PPTX_PATH;
  const zip = await JSZip.loadAsync(fs.readFileSync(writtenPath));
...
async function main() {
  ...
  console.log(JSON.stringify({ pptx: TARGET_PPTX_PATH, copy: COPY_PATH, montage: MONTAGE_PATH, qa: QA_PATH, slides: slides.length, notes: pptxReport.notesInPptx }, null, 2));
}
