...
const OUT_DIR = path.join(ROOT, "output", "defense_ppt_html_raster");
const SLIDE_DIR = path.join(OUT_DIR, "slides");
const DESKTOP = path.join(os.homedir(), "Desktop");
const PPTX_PATH = path.join(DESKTOP, "基于集群的多任务协同态势感知平台_本科毕设答辩_终版图片大字版.pptx");
...
function coverSlide() {
  return `<main class="slide cover">
    ...
    <div class="cover-school-stamp">江西理工大学 · 志存高远 · 责任为先</div>
    <div class="cover-watermark">JXUST</div>
    <div class="cover-cn-watermark">江西理工大学</div>
...
const slides = [
    ...
    html: slideShell(6, "系统需求：面向实验验证的协同感知平台", "平台要让算法可配置、过程可观察、结果可比较，而不是只输出一个最终数字。", `
      <div class="req-v2">
        <section class="req-left">
          ...
          ].map(([h, p], i) => `<div class="req-chip ${i % 2 ? "blue" : "red"}" data-fit><b>${h}</b><span>${p}</span></div>`).join("")}
        </section>
        <section class="req-screen" data-fit>
          ...
          <div class="screen-tags" data-fit>
            <span>场景配置</span><span>策略指标</span><span>三维沙盘</span><span>收益曲线</span><span>任务级分析</span>
          </div>
  ...
  },
  {
    ...
    html: slideShell(15, "感知融合与严格成功：覆盖只是开始，融合结果才是最终评价", "平台中的成功判定同时要求观测确认、定位达标和一致性达标，因此比普通覆盖率更严格。", `
      <div class="fusion-layout">
        <section>
          ...
          ${formula("x<sub>est</sub>=Σ(w<sub>i</sub>x<sub>i</sub>)/Σw<sub>i</sub>；Success=Confirmed ∧ Error≤32m ∧ Consensus≥0.42", "w 由测量方差、链路质量和剩余能量决定；延迟观测按信息年龄衰减。")}
          ${panel("融合逻辑", bullets([
    ...
    notes: [
      ...
      "融合部分采用带权位置估计，权重与测量方差、链路质量和剩余能量有关；延迟观测会按信息年龄衰减，离群观测也会被削弱。",
  ...
  <head>
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      .req-chip b { display: block; color: ${C.navy}; font-size: 26px; margin-bottom: 14px; }
      ...
      .screen-annotated img { width: 100%; height: 100%; object-fit: contain; object-position: center top; filter: saturate(.96) contrast(1.03); }
      ...
      .screen-annotated .t1 { left: 34px; top: 54px; }
      .screen-annotated .t2 { left: 265px; top: 55px; background: rgba(179,58,50,.92); }
      .screen-annotated .t2::after { border-top-color: rgba(179,58,50,.92); }
      .screen-annotated .t3 { left: 255px; top: 235px; }
      .screen-annotated .t4 { right: 112px; top: 170px; background: rgba(179,58,50,.92); }
      .screen-annotated .t4::after { border-top-color: rgba(179,58,50,.92); }
      .screen-annotated .t5 { right: 58px; bottom: 92px; }
      .screen-tags {
        margin-top: 10px;
        display: grid;
